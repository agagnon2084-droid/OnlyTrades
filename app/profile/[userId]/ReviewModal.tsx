    ...
    <Modal open={open} onClose={() => { setOpen(false); router.replace(`/profile/${revieweeId}`); }} title={`Review ${revieweeName}`}>
      <div className="flex flex-col gap-5">
        <p className="text-sm text-[var(--muted-foreground)]">
          How was your trade experience with <strong className="text-[var(--foreground)]">{revieweeName}</strong>?
        </p>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm font-medium text-[var(--foreground)]">Comment (optional)</label>
          <textarea
            value={comment}
